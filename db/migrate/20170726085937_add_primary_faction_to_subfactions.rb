  ...
  def change
    ...
    reversible do |dir|
      dir.up do
        ...
        imperial_id = select_rows("select id from factions where name = 'Galactic Empire'")[0][0]
        if imperial_id.present?
          execute("update factions set primary_faction_id = #{imperial_id} where name = 'First Order'")
        end
        rebels_id = select_rows("select id from factions where name = 'Rebel Alliance'")[0][0]
