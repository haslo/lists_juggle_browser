    ...
    def initialize(ranking_configuration, ship_id: nil, pilot_id: nil, upgrade_id: nil, ship_combo_id: nil, other_ship_combo_id: nil)
      ...
      if ship_combo_id.present? || other_ship_combo_id.present?
        if ship_combo_id.nil?
          ship_combo_query = Squadron.joins(:ship_combos).merge(ShipCombo.where(id: other_ship_combo_id)).select(:id).to_sql
        elsif other_ship_combo_id.nil?
          ship_combo_query = Squadron.joins(:ship_combos).merge(ShipCombo.where(id: ship_combo_id)).select(:id).to_sql
        else
          ship_combo_query = Squadron.joins(:ship_combos).merge(ShipCombo.where('id = ? or id = ?', ship_combo_id, other_ship_combo_id)).select(:id).to_sql
        end
      ...
      SQL
      @games = game_query.all.order(order)
